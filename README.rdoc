...
Fork of the fantastic model_translations by janne. Minimal implementation of Globalize2 style model translations with validates_uniqueness_of for
translated attributes. Rails 2.2 is required.
...
model_translations will detect if the column is translated or not and perform the required lookup, 
adding the usual +:taken+ error to +ActiveRecord::Errors+
